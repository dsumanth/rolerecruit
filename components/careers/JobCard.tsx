...
export function JobCard({ jobId, title, subject, level, qualifications, minExperience, slug }: Props) {
  return (
    <Link
      ...
      className="block rounded-apple bg-surface border border-surface-tertiary p-5 hover:shadow-md transition-shadow"
    >
      <h2 className="text-lg font-semibold text-ink">{title}</h2>
      <div className="flex flex-wrap gap-2 mt-2">
        <span className="text-xs px-2 py-0.5 rounded-full bg-accent/10 text-accent">{subject}</span>
        <span className="text-xs px-2 py-0.5 rounded-full bg-surface-secondary text-ink-secondary">{level}</span>
      </div>
