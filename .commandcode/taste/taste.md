...
- Always use the project's pre-built design tokens (colors, spacing, typography, radii, shadows) for any UI work. Never hardcode raw values or introduce one-off styles that bypass the token system, so the design language stays consistent across the app. Confidence: 0.85
